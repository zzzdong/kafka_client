...
fn generate_decode_field(field: &FieldInfo, flexible_version: Option<i16>) -> TokenStream {
    ...
    let decode_body = generate_decode_body(field, use_flexible);
...
/// 生成字段解码体
fn generate_decode_body(field: &FieldInfo, use_flexible: bool) -> TokenStream {
    // Option 类型特殊处理
    if field.is_option() {
        let inner_decode = generate_decode_body_inner(field, use_flexible);
    ...
    generate_decode_body_inner(field, use_flexible)
}

/// 生成内部类型的解码体
fn generate_decode_body_inner(field: &FieldInfo, use_flexible: bool) -> TokenStream {
    let ty = &field.ty;
    ...
    // 检查是否是 Vec 类型（使用 AST 而非字符串比较）
    if let Some(vec_inner_ty) = FieldInfo::extract_vec_inner(&inner_ty) {
        if use_flexible {
            quote! { decode_compact_array(buf, |b| <#vec_inner_ty as ::kafka_client_protocol_core::Message>::decode(b, version))? }
        } else {
            quote! { decode_array(buf, |b| <#vec_inner_ty as ::kafka_client_protocol_core::Message>::decode(b, version))? }
        }
    } else {
        // 对于非 Vec 类型，使用字符串比较
        ...
        if inner_ty_str == "String" {
            ...
            if use_flexible {
                quote! { decode_compact_bytes(buf)? }
            } else {
                quote! { decode_bytes(buf)? }
            }
