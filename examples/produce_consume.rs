...
use bytes::Bytes;
use kafka_client::{
    AutoOffsetReset, ConsumerConfig, KafkaClient, PartitionAssignmentStrategy,
...
fn get_topic_name() -> String {
    std::env::var("KAFKA_TOPIC")
        .unwrap_or_else(|_| "example-topic".to_string())
}
...
async fn main() {
    ...
    for t in &resp.topics {
        ...
        if t.error_code != 0 && t.error_code != 36 {
            eprintln!("ERROR: Topic creation failed with error code {}", t.error_code);
            std::process::exit(1);
        }
    ...
    for i in 0..3 {
        ...
        match producer.send(record).await {
            Ok(meta) => println!("  Sent to partition {} at offset {}", meta.partition, meta.offset),
            ...
            Ok(recs) => {
                for r in recs {
                    println!(
                        ...
                        r.partition,
                        r.offset,
                        r.key.as_ref().map(|k| String::from_utf8_lossy(k).to_string()),
