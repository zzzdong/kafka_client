...
fn get_sasl_config() -> (SaslMechanismType, String, String) {
    let mechanism = std::env::var("SASL_MECHANISM").unwrap_or_else(|_| "PLAIN".to_string());
    ...
    let username =
        std::env::var("SASL_USERNAME").expect("SASL_USERNAME environment variable required");
    let password =
        std::env::var("SASL_PASSWORD").expect("SASL_PASSWORD environment variable required");
